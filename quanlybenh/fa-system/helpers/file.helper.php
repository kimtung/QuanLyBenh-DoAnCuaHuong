...
if (!function_exists('mmdir'))
{
    ...
    function mmdir($dir) {
        ...
        $year = date("Y");
        $year_dir = $dir . '/' . $year;
        if (!file_exists($year_dir) || !is_dir($year_dir))
        {
            if (!mkdir($year_dir))
            {
                return false;
        ...
        if (!file_exists($month_dir) || !is_dir($month_dir))
        {
            if (!mkdir($month_dir))
            {
                return false;
...
if (!function_exists('get_list_files'))
{
    /**
     * Get list files in a direction
     *
     ...
     * @return	array
     */
    function get_list_files($source_dir, $include_path = FALSE, $_recursion = FALSE)
    {
        ...
        if ($fp = @opendir($source_dir))
        {
            ...
            while (FALSE !== ($file = readdir($fp)))
            {
                ...
                {
                    get_list_files($source_dir.$file.DIRECTORY_SEPARATOR, $include_path, TRUE);
                }
                elseif ($file[0] !== '.')
                {
